"""
Template API routes for the Notification Service.

This module contains all template-related endpoints including:
- Create templates
- Get templates (by name, with version support)
- Update templates
- Delete templates
...
async def get_latest_template(
    ...
    # 2. If miss, get from PostgreSQL
    template = await crud.get_template_by_name(
        db, name=name, language=language
    )
...
async def update_template(
    ...
    latest_template = await crud.get_template_by_name(
        db, name=name, language=language
    )
    ...
    new_version = await crud.create_new_template_version(
        db=db, latest_template=latest_template, template_update=template_update
    )
